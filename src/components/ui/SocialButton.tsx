...
const LinkedInIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
  <svg data-realm-icon="social" className={`fill-current ${className}`} viewBox="0 0 24 24">
  ...
  return (
    ...
    >
      ...
      <div className={`w-full transition-opacity duration-200 ${isVisible ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <button
          type="button"
          {...getUniversalAudioProps('CARD_CLICK', 'CARD_HOVER', () => setIsVisible(true))}
        ...
        {items.map((item, i) => {
          ...
          return (
            <button
              ...
              {...getUniversalAudioProps('CARD_CLICK', 'CARD_HOVER', () => handleChannelClick(i, item))}
              style={{
